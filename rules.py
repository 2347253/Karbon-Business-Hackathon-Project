...
def get_flag_color(flag_value):
    """Return color based on the flag value."""
    if flag_value == FLAGS.GREEN:
        return "green"
    elif flag_value == FLAGS.RED:
        return "red"
    elif flag_value == FLAGS.AMBER:
        return "orange"
    else:   
        return "gray"  
...
def total_borrowing(data: dict, financial_index):
    ...
    try:
            ...
            long_term_borrowings = float(liabilities.get("long_term_borrowings", 0.0))
            short_term_borrowings = float(liabilities.get("short_term_borrowings", 0.0))
        ...
        print(f"Error calculating total borrowing ratio: {str(e)}")
        return 0.0
...
def total_revenue_5cr_flag(data: dict, financial_index):
    ...
    if revenue >= 50000000:  # 50 million or 5 CRORes
        return FLAGS.GREEN
    else:
        return FLAGS.RED
...
def iscr(data: dict, financial_index: int):
    ...
    if financial_index < len(financials):
        ...
        # Extract EBIT and interest
        ebit = line_items.get("profit_before_interest_and_tax", 0)
        interest = line_items.get("interest", 0)

        # Add 1 to avoid division by zero
        iscr_value = (ebit + 1) / (interest + 1)
        return iscr_value
